...
export async function GET(
    ...
    try {
        ...
        if(!sizeId) return NextResponse.json({ msg:'size id is required', status:400});
        ...
        return NextResponse.json({ msg:`Error in Size GET req ${error}`,status:500});
    }
}
...
export async function PATCH(
    ...
    try {
        ...
        if( !value ) return NextResponse.json({ msg: 'Size value is required',status:400});
    ...
    return NextResponse.json({ msg:`Error in Size PATCH req ${error}`, status:500})
...
try {
    ...
    return NextResponse.json({ msg:`Error in size DELETE req ${error}`,status:500});
}
